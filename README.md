# FlyRank AI Fluency internship

Deliverables for the General AI Fluency track, one folder per assignment card.

The portfolio these assignments build is a separate repo: **[emircabalak.github.io](https://github.com/emircabalak/emircabalak.github.io)**, live at <https://emircabalak.github.io/>.

## Week 1

| Card | Deliverable |
|---|---|
| What Are You Proving? | [proof-statement.md](W1-01-what-are-you-proving/proof-statement.md) |

## Week 2

| Card | Deliverable |
|---|---|
| Frame It as Cases | [framed-cases.md](W2-01-frame-it-as-cases/framed-cases.md) |
| The Prompt Ladder | [prompt-ladder.md](W2-02-the-prompt-ladder/prompt-ladder.md) |
| Prompting Fundamentals on Real Tasks v2 (FL-02) | [prompt-iteration-log.md](W2-03-FL-02-prompting-fundamentals/prompt-iteration-log.md) |

## Week 3

| Card | Deliverable |
|---|---|
| Decide Once: Build Your Identity Kit | [identity-kit.pdf](W3-01-identity-kit/identity-kit.pdf), plus [identity-kit.md](W3-01-identity-kit/identity-kit.md), [logo.svg](W3-01-identity-kit/logo.svg), [favicon.svg](W3-01-identity-kit/favicon.svg), [tokens.css](W3-01-identity-kit/tokens.css) |
| Kill your darlings: Curate Your Images | [image-set.pdf](W3-02-curate-your-images/image-set.pdf), plus [image-set.md](W3-02-curate-your-images/image-set.md), the four SVGs and the portrait |
| The Through-Line: Map Content and CTAs | [content-map.md](W3-03-through-line/content-map.md) |
| Consistency, Not Talent (and Frame, Not Upstage) | [week-3-package.pdf](W3-04-consistency-not-talent/week-3-package.pdf) |

## Week 4

| Card | Deliverable |
|---|---|
| Empty but Live: Ship a Blank Page | [empty-but-live.md](W4-01-empty-but-live/empty-but-live.md) |
| Three Roads: Choose Your Stack with AI | [stack-rationale.md](W4-02-three-roads/stack-rationale.md) |
| Ship an Automation Workflow v2 (FL-04) | [walkthrough.md](W4-03-FL-04-automation-workflow/walkthrough.md), [workflow-config.md](W4-03-FL-04-automation-workflow/workflow-config.md) |
| Agent Concepts and MCP Basics (FL-05) | [explainer.md](W4-04-FL-05-agent-concepts-mcp/explainer.md), [mcp-evidence.md](W4-04-FL-05-agent-concepts-mcp/mcp-evidence.md) |

## Week 5

| Card | Deliverable |
|---|---|
| Ship the Ugly One | [ship-the-ugly-one.md](W5-01-ship-the-ugly-one/ship-the-ugly-one.md), plus the live URL <https://emircabalak.github.io/> |
| Explain It Like You Built It | [how-deploying-works.md](W5-05-explain-it-like-you-built-it/how-deploying-works.md) |
| Design Your Personal Agent (FL-06) | [agent-spec.md](W5-02-FL-06-design-your-agent/agent-spec.md) |
| Build the Agent (FL-07) | [BUILD-LOG.md](W5-03-FL-07-build-the-agent/BUILD-LOG.md), [AGENT.md](W5-03-FL-07-build-the-agent/agent/AGENT.md), [results.md](W5-03-FL-07-build-the-agent/evals/results.md), [case1_pair.md](W5-03-FL-07-build-the-agent/reports/case1_pair.md) |
| Personal Website Live on the FlyRank Domain (PF-04) | [site-and-hosting.md](W5-04-PF-04-website-live/site-and-hosting.md) and [dns-walkthrough.md](W5-04-PF-04-website-live/dns-walkthrough.md), plus the live URL <https://emircabalak.github.io/> |

## Week 6

| Card | Deliverable |
|---|---|
| Open It on Your Phone | [fix-log.md](W7-01-open-it-on-your-phone/fix-log.md), plus the live URL <https://emircabalak.github.io/> |
| Make It Do Something | [explainer.md](W6-03-make-it-do-something/explainer.md), plus the working feature at <https://emircabalak.github.io/checker.html> |
